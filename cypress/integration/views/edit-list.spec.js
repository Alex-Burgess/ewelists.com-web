...
describe('Edit List E2E Tests', () => {
  const email = "eweuser8+editlist@gmail.com"
  let userId = ""
  let listId = ""

  before(() => {
    cy.exec('python ' + Cypress.config().createUserScript + ' -e ' + email + ' -n "Cypress EditList"').then((result) => {
      userId = result.stdout

      cy.exec('python ' + Cypress.config().createListScript + ' -u ' + userId).then((result) => {
...
describe('Edit List Page Visual Regression tests', () => {
  ...
  const email = "eweuser8+editlist2@gmail.com"
  let userId = ""
  let listId = ""

  before(() => {
    cy.exec('python ' + Cypress.config().createUserScript + ' -e ' + email + ' -n "Cypress EditList2"').then((result) => {
      userId = result.stdout
...
describe('Social Links', () => {
  const email = "eweuser8+editlist3@gmail.com"
  let userId = ""
  let listId = ""

  before(() => {
    cy.exec('python ' + Cypress.config().createUserScript + ' -e ' + email + ' -n "Cypress EditList3"').then((result) => {
      userId = result.stdout
  ...
  it('Should have correct email link', () =>{
    const mailto = 'mailto:?subject=Cypress EditList3 shared a gift list with you&body=Hi!%0D%0A%0D%0AYou can view Baby Gift List at the link below if you wish to buy a gift:%0D%0A%0D%0Ahttp://localhost:3000/lists/' + listId
...
describe('Product Row Tests', () => {
  const email = "eweuser8+editlist4@gmail.com"
  let userId = ""
  let listId = ""

  before(() => {
    cy.exec('python ' + Cypress.config().createUserScript + ' -e ' + email + ' -n "Cypress EditList4"').then((result) => {
      userId = result.stdout
